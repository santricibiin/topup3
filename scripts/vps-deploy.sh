...
if [[ -d "$WA_DIR" ]]; then
  ...
  echo -e "    1. Login admin → sidebar ${BLUE}Konfigurasi OTP${NC} (${BLUE}/admin/waotp${NC})"
  echo -e "    2. URL & API Key sudah auto-terisi (sync dari DB)."
  echo -e "    3. Toggle ${GREEN}Aktifkan OTP WhatsApp${NC} → ${BLUE}Save${NC}"
  echo -e "    4. Section ${BLUE}Sesi WhatsApp${NC} → klik ${GREEN}Connect${NC} → scan QR"
  echo ""
